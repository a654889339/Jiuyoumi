...
async function sendVerificationCode(email) {
  if (!config.email.enabled) throw new Error('邮件服务未启用');
  ...
  verificationCodes.set(email, { code, expiresAt: Date.now() + CODE_EXPIRE_MS, attempts: 0, used: false, lockedUntil: 0 });
...
function verifyCode(email, code) {
  ...
  if (vc.lockedUntil > Date.now()) return { valid: false, message: '验证次数过多，请稍后再试' };
  if (vc.attempts >= 5) { vc.lockedUntil = Date.now() + 30 * 60 * 1000; vc.attempts = 0; return { valid: false, message: '验证次数过多，已锁定 30 分钟' }; }
  vc.attempts++;
  if (vc.used) return { valid: false, message: '验证码已使用，请重新获取' };
  if (Date.now() > vc.expiresAt) { verificationCodes.delete(email); return { valid: false, message: '验证码已过期，请重新获取' }; }
  if (vc.code !== code) return { valid: false, message: '验证码错误' };
  vc.used = true;
  return { valid: true };
}
