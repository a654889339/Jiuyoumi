...
const CODE_EXPIRE_MS = (config.sms && config.sms.codeExpireMinutes ? config.sms.codeExpireMinutes : 5) * 60 * 1000;
...
async function sendVerificationCode(phone) {
  ...
  verificationCodes.set(key, { code, expiresAt: Date.now() + CODE_EXPIRE_MS, used: false });
...
function verifyCode(phone, code) {
  ...
  if (Date.now() > vc.expiresAt) { verificationCodes.delete(key); return { valid: false, message: '验证码已过期，请重新获取' }; }
  if (vc.code !== String(code).trim()) return { valid: false, message: '验证码错误' };
  vc.used = true;
  return { valid: true };
}
